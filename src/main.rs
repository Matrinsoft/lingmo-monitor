                ...
                .unwrap_or_default(),
            {
                ...
                }
                {
                    ...
                    if let Some(temp) = graph_item.max_cpu_temp() {
                        row = row.push(self.tag(
                            widget::text::body(format!("{:.1}掳C", temp)),
...
impl Application for App {
    ...
    const APP_ID: &'static str = "com.lingmoos.LingmoMonitor";
    ...
    fn init(mut core: Core, flags: Self::Flags) -> (Self, Task<Self::Message>) {
        ...
        let about = About::default()
            ...
            .links([
                (
                    fl!("repository"),
                    "https://github.com/Matrinsoft/lingmo-monitor",
                ),
                (
                    fl!("support"),
                    "https://github.com/Matrinsoft/lingmo-monitor/issues",
                ),
            ]);
                        ...
                        if let Some(temp) = graph_item.max_cpu_temp() {
                            row = row.push(self.tag(
                                widget::column!(
                                    ...
                                    widget::text::heading(format!("{:.1}掳C", temp))
                                ),
                                Message::CpuGraph(ProcGraphKind::Temperature),
                                true,
                            ));
